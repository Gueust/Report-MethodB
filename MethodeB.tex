...
\subsubsection{Informellement parlant}
...
\subsubsection{Formellement parlant}

Soit une substitution S et une opération "op" la réalisant. Dans la clause \textbf{OPERATIONS} on trouve :
\begin{itemize}
...
\begin{itemize}
\item Des préconditions sous le mot-clé \textbf{PRE} P \textbf{THEN} \emph{stuff} \textbf{END}
(ces préconditions s'ajoutent aux invariants, toujours vrais au moins jusqu'à avant l'opération).
\item Un choix dans un ensemble avec \textbf{ANY} x \textbf{WHERE} P(x) \textbf{THEN} \emph{stuff with x} \textbf{END}.
\item Des boucles avec \textbf{WHILE} P(x) \textbf{DO} \emph{stuff} \textbf{END}.
\end{itemize}
