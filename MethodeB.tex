...
Voici les clauses rédigées en B (ici avec des symboles mathématiques, au lieu des charactères ASCII qui les réprésente en réalité \cite[p. 29]{introB}).
\\
\\
\\
...
Que doit-on prouver ? Ici, les invariants potentiellement affectés par l'opération sont :
\begin{itemize}
\item emprunt $\in$ exemplaire $\pfun$ abonne
\item $\forall$ ab\ (\ ab $\in$ abonne $\Rightarrow$ (\ \textbf{card}\ (\ emprunt$^{-1}(\{ab\}) \leq$ maxi\ ))
\end{itemize}~

La précondition $ll \mapsto nn \not\in $ dom(emprunt) permet de s'assurer que le nouvel ensemble emprunt reste une fonction partielle, ce qui prouve que emprunt $\in$ exemplaire $\pfun$ abonne après l'opération.

Ensuite, puisque le cardinal de emprunt$^{-1}(\{ab\})$ est strictement plus petit que maxi et qu'on lui rajoute un unique élément, le deuxième invariant est également vérifié.
